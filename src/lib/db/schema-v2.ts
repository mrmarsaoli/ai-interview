...
export const messages = sqliteTable('messages', {
  ...
  sessionOrderIdx: index('messages_session_order_idx').on(table.sessionId, table.orderIndex),
  
  // Index for timestamp-based queries (fallback ordering)
  sessionTimeIdx: index('messages_session_time_idx').on(table.sessionId, table.createdAt),
  
  // Index for role-based filtering (e.g., last AI message)
  sessionRoleIdx: index('messages_session_role_idx').on(table.sessionId, table.role),
}));
